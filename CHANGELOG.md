...
### Added
- **`nexus pull` removes the projection of the runtime a project no longer uses.** `agent_owner` (`opencode` | `claude-cli`) can be switched at any time: after writing the selected projection, pull removes what Nexus had put on disk for the other runtime (new `projection_cleanup` module). It runs when the cached owner changed and whenever leftovers are still on disk (e.g. repos switched with an earlier CLI). This supersedes the 0.27.0 behaviour ("the stale-projection warning stays, nothing is deleted").
  - Switched to Claude Code: `.opencode/` commands, plugins and `package.json` Nexus recorded (pull manifest, sync manifest, rendered export content), OpenCode install artifacts (`node_modules/`, `package-lock.json`, `bun.lock`, `.gitignore`, `.DS_Store`). `opencode.json` loses the Nexus parts only: the operator's own `mcp` servers and `instructions` outside the agentic root are kept (the file goes when nothing else is left; a committed file is left alone without `--force`); `opencode.jsonc`, which Nexus never writes, only with `--force`.
  - Switched to OpenCode: CCX lock files and hook scripts (existing orphan rule), `.claude/skills/**` and `.claude/agents/*.md` Nexus rendered, the routing-guard inputs in `<agentic_root>/generated/`, the Nexus keys in `.claude/settings.json` (lock-recorded keys, hook registrations, baseline permissions, `includeCoAuthoredBy`, routing env; the file is deleted when only `$schema` remains), the Nexus servers in `.mcp.json`, the `CLAUDE.md` managed block (the file is deleted when only the untouched bootstrap template remains), then the CCX lock and empty directories. `.claude/settings.local.json` and files Nexus never wrote are never touched; `.claude/` goes only when empty.
  - Unmodified Nexus files are deleted. Locally modified ones, and files that only carry the `source: nexus-platform` marker, a registry plugin name or an actor frontmatter without a hash record (an edit cannot be ruled out), are kept and listed with a `--force` hint; a `CLAUDE.md` block the CCX lock does not record as written unchanged is kept too. `--force` / `--force-unmanaged` also remove them (and, for OpenCode, all of `.opencode/`); `-y` alone never does. `--force` on the `CLAUDE.md` bootstrap template keeps other tools' managed blocks (e.g. `next dev`'s `nextjs-agent-rules`). Removed files are dropped from the sync and pull manifests (kept ones stay recorded, a partial CCX lock keeps kept CCX files), so switching back recreates everything. Output: a `Projection cleanup:` section (`- path (projection removed)`, `~ path (Nexus settings removed)`, `! path (kept: modified locally)`); a second pull is a no-op.
  - Never deletes or rewrites through a symlinked directory (e.g. `.claude/skills` linked to a shared dotfiles tree).
  - Skipped when the backend could not tell the owner (unreachable, or no owner reported while the previous pull recorded Claude Code) or the `opencode.json` write was declined; a cleanup error is reported without aborting the pull.
- **Git hook self-heal.** At the end of every `nexus pull`, a repo shipping `.githooks/pre-commit` gets `core.hooksPath` set to `.githooks` when it is unset in every git config scope; a value set on purpose (global company hooks, husky, ...) is never overridden and only gets a hint. Warns when the hook runs `gitleaks` but gitleaks is not installed. With `.pre-commit-config.yaml` instead, pull only hints when its hook is not installed. Outside a git repository nothing happens.
- **Workspace overwrite guard.** When `devbox.json` or a `scripts/devbox/**` file is tracked in git, the local file equals `HEAD`, differs from what Nexus last delivered and the workspace fork has a different version, pull keeps it ("committed locally and differs from the Nexus workspace fork"; compare with `nexus diff`, then `nexus push` or `nexus pull --force`). A committed file that still equals the version Nexus last delivered is updated normally when the fork moved on. With `--force` (not `-y`) a guarded file is overwritten and listed in a prominent warning suggesting `git diff` / `nexus push`.

### Fixed
- **`permissions.deny` (and every nested managed setting) was never applied.** The backend sends `claude_settings.values` nested (`{"permissions": {"deny": [...]}}`), while merge, removal reconciliation, `status`/`diff` and the cleanup looked values up only by the dotted key `"permissions.deny"`. `ClaudeSettingsSpec::value()` resolves both forms; the next pull adds `Read(./.env)` / `Read(./.env.*)`.
- **Hook matchers were never updated.** An existing Nexus hook registration kept the matcher of the pull that first wrote it, so widened matchers (e.g. `mcp__nexus__.*` for session-guard and headroom-intercept) never took effect. A registration that holds exactly the Nexus adapter command now follows the backend matcher (entries the operator changed or extended stay untouched); removal matches the recorded command regardless of the matcher, so no registration is left pointing at a deleted hook script.
- **`nexus push` uploaded untracked scripts.** Files reported as "not tracked by Nexus, skipped" were still sent into the new workspace fork; only the tracked files are uploaded now.
- Agent files of the other runtime are skipped correctly with the legacy agentic root `.claude` (only `.claude/agents/` is Claude-only there).

### Changed
- `nexus status` reports leftovers of the unused runtime (not the mere presence of `.claude/`, which may hold the operator's own files) as STALE with the next action "run nexus pull to remove it"; `nexus reset` on it points to `nexus pull`.
- Agent files of the other runtime are skipped in both directions (pull and init; previously only `.opencode/**` for Claude Code projects). CCX files are applied and reported only for Claude Code projects, so an OpenCode project no longer gets `.claude/rules` etc.
- `nexus init` creates `.opencode/commands/` and installs OpenCode plugins only for OpenCode projects (and in the offline default).
- Rendered `.claude/skills/**` and `.claude/agents/*.md` and registry-downloaded OpenCode plugins are recorded in the pull manifest, so a later switch can tell them from local edits. `.mcp.json` / `opencode.json` get the Nexus server back when it is missing, and the routing-guard `env` keys are re-added to an existing `.claude/settings.json`.
- **Workspace files ignore trailing-newline-only and JSON key-order differences** in pull, status, diff, push and reset: a `devbox.json` committed with a final newline by an end-of-file fixer no longer flip-flops on every `nexus pull --force`, and a freshly pushed `devbox.json` (the backend returns it with its own key order) is no longer reported as UPDATE. Written workspace files end with exactly one newline. The v1 workspace fallback now skips locally modified files like v2 (one shared `sync_workspace_file`).

Verified end to end on a copy of test repo 701 (staging): `claude-cli` pull removing old `.opencode/` leftovers, switch to `opencode` and back, `--force`, each followed by a no-op pull. 44 new tests; 716/716 passing, clippy/fmt clean.
