...
pub async fn diff(api_url: &str, path: Option<&str>) -> anyhow::Result<i32> {
    ...
    for e in entries.iter().filter(|e| e.state != State::Stale) {
        let desired_label = match e.class {
            FileClass::Content => "backend",
            FileClass::Projection => "nexus",
        };
        print!(
            "{}",
            unified_diff(
                e.local.as_deref().unwrap_or(""),
                e.desired.as_deref().unwrap_or(""),
                &format!(
                    "{} (local, {} {})",
                    e.path,
                    class_label(e.class),
...
fn refuse_projection(e: &Entry) -> Option<String> {
    ...
    Some(match setting {
        ...
        None => format!(
            "{} is generated by Nexus and cannot be pushed; discard the local edit with nexus reset {} (skills are managed in the Nexus dashboard)",
            e.path, e.path
        ),
    })
}
