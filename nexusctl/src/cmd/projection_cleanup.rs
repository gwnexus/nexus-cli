...
enum Origin {
    ...
    Modified,
    /// Nexus-owned without a hash record (marker, registry plugin name).
    Owned,
    /// Not written by Nexus.
    Unknown,
}
...
fn plan_opencode(workspace: &Path, ctx: &CleanupContext, records: &Records) -> Vec<Action> {
        ...
        if ctx.force && !ctx.keep.iter().any(|k| k.starts_with(".opencode/")) {
            ...
            for rel in files {
                ...
                actions.push(match records.origin(&rel, &bytes, registry_plugin) {
                    Origin::Pristine | Origin::Owned => Action::RemoveFile {
                        path: rel,
                        note: "projection removed",
                    },
                    Origin::Modified => Action::Keep {
                        path: rel,
    ...
    // opencode.json(c): generated by Nexus on every pull and git-excluded
    // (it carries the token), so Nexus-owned unless someone committed it.
    for rel in OPENCODE_ROOT_FILES {
        if !workspace.join(rel).is_file() || kept_by_pull(rel) {
            continue;
        }
        if !ctx.force && is_git_tracked(workspace, rel) {
...
fn plan_claude(
    ...
    for rel in files {
        ...
        match records.origin(&rel, &bytes, false) {
            Origin::Pristine | Origin::Owned => actions.push(Action::RemoveFile {
                path: rel,
                note: "projection removed",
            }),
            Origin::Modified => actions.push(remove_or_keep(rel, false)),
...
/// `force` also when a modified bootstrap template remains.
pub(crate) fn strip_claude_md(
    ...
    let (rest, had_block) = match (content.find(begin), content.find(end)) {
        (Some(b), Some(e)) if e > b => {
            ...
            if locked_block_sha.is_some_and(|k| sha256_hex(block) != k) && !force {
...
mod tests {
    ...
    fn test_strip_claude_md_variants() {
        ...
        assert_eq!(
            strip_claude_md(&with_block(&template), None, &template, false),
            ClaudeMdCleanup::Remove
        );
        // Block only.
        assert_eq!(
            strip_claude_md(&with_block(""), None, &template, false),
            ClaudeMdCleanup::Remove
        );
        ...
        assert_eq!(
            strip_claude_md(&middle, None, &template, false),
        ...
        assert_eq!(
            strip_claude_md(&with_block(&edited), None, &template, false),
            ClaudeMdCleanup::Rewrite(edited.clone())
        );
        assert_eq!(
            strip_claude_md(&with_block(&edited), None, &template, true),
            ClaudeMdCleanup::Remove
        );
